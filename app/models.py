...
class Block(Base):
    ...
    __table_args__ = (
        UniqueConstraint("pipeline_id", "name", name="uq_block_name_in_pipeline"),
    )
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    ...
    incoming_edges: Mapped[List["Edge"]] = relationship(
        ...
        foreign_keys="Edge.to_block_id",
        cascade="all, delete-orphan",
    )
    outgoing_edges: Mapped[List["Edge"]] = relationship(
        back_populates="from_block",
        foreign_keys="Edge.from_block_id",
        cascade="all, delete-orphan",
    )
...
class Edge(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "pipeline_id", "from_block_id", "to_block_id", name="uq_edge_unique"
        ),
        Index("ix_edges_from_to", "from_block_id", "to_block_id"),
    )
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    pipeline_id: Mapped[int] = mapped_column(
        ForeignKey("pipelines.id", ondelete="CASCADE"), nullable=False, index=True
    ...
    to_block_id: Mapped[int] = mapped_column(
        ForeignKey("blocks.id", ondelete="CASCADE"), nullable=False
    )

    pipeline: Mapped["Pipeline"] = relationship(back_populates="edges")
    from_block: Mapped["Block"] = relationship(
        back_populates="outgoing_edges", foreign_keys=[from_block_id]
    )
    to_block: Mapped["Block"] = relationship(
        back_populates="incoming_edges", foreign_keys=[to_block_id]
    )
...
class PipelineRun(Base):
    ...
    block_runs: Mapped[List["BlockRun"]] = relationship(
...
class BlockRun(Base):
    ...
    artifacts: Mapped[List["Artifact"]] = relationship(
        back_populates="block_run", cascade="all, delete-orphan"
    )
...
class Artifact(Base):
    ...
    block_run_id: Mapped[int | None] = mapped_column(
        ForeignKey("block_runs.id", ondelete="SET NULL"), nullable=True, index=True
    )
    kind: Mapped["ArtifactKind"] = mapped_column(
        SAEnum(ArtifactKind), default=ArtifactKind.GENERIC, nullable=False
    )
...
class BlockQueue(Base):
    ...
    __table_args__ = (
        Index("ix_block_queue_priority", "priority"),
        Index("ix_block_queue_enqueued_at", "enqueued_at"),
    )
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    pipeline_run_id: Mapped[int] = mapped_column(
        ForeignKey("pipeline_runs.id", ondelete="CASCADE"), nullable=False, index=True
...
class LogRecord(Base):
    ...
    pipeline_run_id: Mapped[int | None] = mapped_column(
        ForeignKey("pipeline_runs.id", ondelete="CASCADE"), nullable=True, index=True
    )
    block_run_id: Mapped[int | None] = mapped_column(
        ForeignKey("block_runs.id", ondelete="SET NULL"), nullable=True, index=True
    )
...
class PipelineHistory(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    pipeline_name: Mapped[str] = mapped_column(String(200), index=True, nullable=False)
    version: Mapped[int] = mapped_column(Integer, nullable=False)
    spec_json: Mapped[dict] = mapped_column(JSON, nullable=False)
    created_at: Mapped[datetime] = mapped_column(
        DateTime, default=func.now(), nullable=False
    )
    __table_args__ = (Index("ix_ph_name_version", "pipeline_name", "version"),)
