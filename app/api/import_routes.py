...
def _validate_spec(spec: PipelineImportIn) -> List[str]:
    ...
    for n in names:
        if state[n] == 0: dfs(n)
        if cycle: break
    if cycle: errors.append("Graph contains a cycle.")
    return errors
...
def _parse_yaml_or_json(body: Any) -> PipelineImportIn:
    ...
    if isinstance(body, str):
        try:
            ...
            raise HTTPException(status_code=400, detail="YAML input provided but PyYAML not installed.")
        try:
    ...
    raise HTTPException(status_code=400, detail="Unsupported import body; provide a JSON object or YAML string.")

@router.post("/pipelines/import", response_model=PipelineImportOut)
def import_pipeline(spec: Any = Body(...), dry_run: bool = Query(default=False), db: Session = Depends(get_db)):
    ...
    if dry_run:
        return PipelineImportOut(pipeline_id=0, created_blocks=len(parsed.blocks), created_edges=len(parsed.edges), replaced=False)
    replaced = False
    existing = db.execute(select(models.Pipeline).where(models.Pipeline.name == parsed.name)).scalar_one_or_none()
    if existing and parsed.replace_if_exists:
        db.delete(existing); db.commit()
        replaced = True
    elif existing and not parsed.replace_if_exists:
        raise HTTPException(status_code=409, detail=f"Pipeline with name '{parsed.name}' already exists.")
    p = models.Pipeline(name=parsed.name)
    db.add(p); db.flush()
    name_to_id = {}
    for b in parsed.blocks:
        cfg = BLOCK_CFG_MODELS[b.type](**(b.config or {})).dict()
        blk = models.Block(pipeline_id=p.id, type=models.BlockType[b.type], name=b.name, config_json=cfg)
        db.add(blk); db.flush()
        name_to_id[b.name] = blk.id
    for e in parsed.edges:
        db.add(models.Edge(pipeline_id=p.id, from_block_id=name_to_id[e.from_], to_block_id=name_to_id[e.to]))
    db.commit()
    Scheduler(db).validate_dag(p.id)
    return PipelineImportOut(pipeline_id=p.id, created_blocks=len(parsed.blocks), created_edges=len(parsed.edges), replaced=replaced)
