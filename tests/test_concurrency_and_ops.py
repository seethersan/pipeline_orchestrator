...
def _make_two_roots_pipeline(session: Session):
    ...
    b1 = models.Block(pipeline_id=p.id, type=models.BlockType.CSV_READER, name="csv1", config_json={"input_path": "missing1.csv"})
    b2 = models.Block(pipeline_id=p.id, type=models.BlockType.CSV_READER, name="csv2", config_json={"input_path": "missing2.csv"})
    session.add_all([b1,b2]); session.commit()
    return p
...
def test_progress_endpoint(tmp_path):
    ...
    try:
        ...
        db.add(p); db.flush()
        b = models.Block(pipeline_id=p.id, type=models.BlockType.CSV_READER, name="csv", config_json={"input_path": str(csvp)})
        db.add(b); db.commit()
        run = Orchestrator(db).start_run(p.id)
