...
def test_scheduler_initial_enqueues_roots():
    ...
    try:
        ...
        assert enq == 1  # only the CSV reader is root
        q = db.scalars(select(models.BlockQueue).where(models.BlockQueue.pipeline_run_id == run.id)).all()
        assert len(q) == 1
        br = db.scalars(select(models.BlockRun).where(models.BlockRun.pipeline_run_id == run.id)).all()
        assert len(br) == 1
    finally:
        db.close()
...
def test_scheduler_enqueues_after_finish():
    ...
    try:
        ...
        assert enq2 == 2  # sentiment & toxicity
        q = db.scalars(select(models.BlockQueue).where(models.BlockQueue.pipeline_run_id == run.id)).all()
        assert len(q) == 3  # 1 initial + 2 new
