...
def recent_logs(
    run_id: Optional[int] = None, limit: int = 100, db: Session = Depends(get_db)
):
    if run_id is not None:
        q = (
            select(models.LogRecord)
            .where(models.LogRecord.pipeline_run_id == run_id)
            .order_by(desc(models.LogRecord.created_at))
            .limit(limit)
        )
    else:
        q = (
