...
class Block(Base):
    ...
    incoming_edges: Mapped[List["Edge"]] = relationship(
        back_populates="to_block", foreign_keys="Edge.to_block_id", cascade="all, delete-orphan"
    )
    outgoing_edges: Mapped[List["Edge"]] = relationship(
        back_populates="from_block", foreign_keys="Edge.from_block_id", cascade="all, delete-orphan"
    )
...
class Edge(Base):
    __tablename__ = "edges"
    __table_args__ = (
        UniqueConstraint("pipeline_id", "from_block_id", "to_block_id", name="uq_edge_unique"),
...
class BlockRun(Base):
    ...
    __table_args__ = (
        Index("ix_block_runs_status", "status"),
    )
...
class Artifact(Base):
    ...
    uri: Mapped[str] = mapped_column(String(500), nullable=False)  # file path or blob url
